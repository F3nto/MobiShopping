...
const hotestItemScreen = ({navigation}) => {

    const dispatch = useDispatch()

    const [hotestProduct, setHotestProduct] = useState([])

    useEffect(() => {

        const getHotestProduct = async() => {
    ...
    const clickPlus = async (item) => {
        ...
        console.log('Selected Plus Product....', item)
    ...
    const saveToCart = (currentItem) => {
        ...
        if(cartData == null){
            ...
            AsyncStorage.setItem('cartTotalQty', JSON.stringify(1))
            dispatch(totalQtyAction.setTotalQty(1))
        
        }else {

            let isInCartId = null

            let totalQty = currentItem.qty

            for(let i = 0; i < cartData.length; i++){

                totalQty += cartData[i].qty

                if(cartData._id == currentItem._id){

                    cartData[i].qty += 1

                    isInCartId = currentItem._id
...
return(
    ...
    <SafeAreaView style = {styles.container}>
        ...
        <HeaderComponent navigation= {navigation} title = 'Hotest Items' icon = 'menu'/>
        ...
        <TouchableOpacity key = {index} onPress={() => {navigation.navigate('DetailScreen', {product: item, parentScreen: 'HotestItemScreen'})}} style = {styles.card} >
...
export default hotestItemScreen;
